...
public class EmailConfig {
	 ...
	 * 
	 * ## 1. SMTP 동작방식 1. Spring Boot 어플리케이션에서 내가 이메일 보냄 2. Google SMTP 서버로
	 * 전송(Google 서버가 중간 배달자 역할) 3. Google SMTP 서버가 이메일 주소에 맞는 "수신자"에게 이메일 서버(Gmail,
	 * yahoo, 회사 이메일 서버, ...)를 통해 발송 4. 최종 수신자가 이메일을 받음
	 * 
	 * (참고) 기본 포트 25를 사용하지만, 보안 강화한 포트 597이나 467가 자주 사용됨
	 */

	// @Value : properties에 작성된 내용 중 key가 일치하는 vlaue값을 얻어와 필드에 대입
	...
	public JavaMailSender javaMailSender() {
		...
		return mailSender;
		// 위처럼 각종 설정이 적용된 JavaMailSender를 Bean우로 등록하여
		// Spring 애플리케이션에서 이메일을 보내기 위한 구성을 제공함
		// 필요한 곳에서 의존성 주입을 받아 사용하면 된다.
	}
}
