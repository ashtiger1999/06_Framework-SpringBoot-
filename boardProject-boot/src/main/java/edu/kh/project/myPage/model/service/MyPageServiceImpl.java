...
public class MyPageServiceImpl implements MyPageService {
	...
	// 비밀번호 변경 서비스
	@Override
	public int changPw(Map<String, String> paramMap, int memberNo) {
		...
		// 입력받은 현재 비밀번호와(평문)
		// DB에서 조회한 비밀번호(암호와)를 비교
		// -> bycrypt.matches(평문, 암호화 비밀번호) 사용
		...
		// 새 비밀번호를 암호화(bycrypt.encode(평문))
		String encPw = bcrypt.encode(paramMap.get("newPw"));
		
		// 3. DB에 업데이트
		// SQL에 전달 해야하는 데이터 2개 (암호화한 새 비번 : encPw, 회원 번호 : memberNo)
		// -> mapper에 전달할 수 있는 전달인자는 단 1개
		// => 묶어서 전달 (paramMap 재활용)
		
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo + ""); // (number) + "" => 문자열
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 클라이언트가 파일이 저장된 폴더에 접근할 수 있는 주소(정적 리소스 요청 주사)
		...
		// 3. DTO 객체를 DB에 전달하기(INSERT)
		int result = mapper.insertUploadFile(uf);
		
		// 4. 삽입 성공 시 파일을 지정된 서버 폴더에 저장
		if(result == 0) return 0;
		
		// folderPath경로 (C:/uploadFiles/test/변경된 파일명)으로
		// 파일을 서버 컴퓨터에 저장
