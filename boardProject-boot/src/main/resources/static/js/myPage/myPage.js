...
if (updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        if (currentNickname !== memberNickname.value) {
            ...
            // await : 비동기 작업의 결과를 기다릴 때 사용하는 키워드
        ...
        // 위의 모든 검증을 통과했을때만 폼 제출
        updateInfo.submit();

        /*
            submit 이벤트 핸들러 함수 내에게서 fetch를 이용한 비동기 함수를 실행 중일 때
            e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form이 submit되어버린 다음일 수 있음
            -> 비동기 작업 중에 form의 기본 제출 동작이 일어나는 가능성이 생김
        */
    });
}
...
if(profileForm != null) {
    ...
    imageInput.addEventListener("change",()=>{
        ...
        if(file) { // 파일이 선택된 경우
            ...
            if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우(정상인 경우)
                ...
                profileImg.src = newImageUrl; // 미리보기 이미지 설정
                                              // (img 태그의 src에 선택한 파일의 임시 경로를 대입)

                previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장(바뀔 일의 대비)
                previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장(바뀔 일의 대비)
        ...
        } else { // 파일이 선택되지 않은 경우(취소를 누른 경우, undefined)
            // 이전 미리보기 이미지로 복원
            ...
            if(previousFile) { // 이전 파일이 존재한다면
                ...
                // DataTransfer를 이용하여 현재 File 객체를 FileList로 변환하여 할당 진행
                dataTransfer.items.add(previousFile);
                imageInput.files = dataTransfer.files; // FileList 반환 후 대입
            }
        }
    })
