...
public class ParameterController {
	 ...
	 * - 클라이언트 요청 시 생성
	 * 
	 * Spring의 Controller 메서드 작성 시 매개변수에 원하는 객체를 작성하면 존재하는 객체를 바인딩 또는 없으면 생성해서 바인딩
	 * 
	...
	public String paramTest1(HttpServletRequest req) {
		 ...
		 * 
		 * - Controller 메서드 반환값에 "redirect:요청주소"
		 * 
		 */
	 ...
	 * -> "" (빈문자열)일 때는 에러 발생 X 
	 * => 파라미터가 존재하지 않는 것이 아니라 name속성값=""(빈문자열)로 넘어오기 때문
	 * 
	...
	public String paramTest2(@RequestParam("title") String title, 
							 ...
							 // 형변환이 필요한 경우에는 파라미터의 value값이 필수 작성되어있어야 함
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		//@RequestParam Map<String,Object> paramMap
		// -> 제출된 모든 파라미터가 하나의 Map에 저장된다.
		// -> 같은 name속성값을 가진 파라미터는 String 배열이나 List로 저장되는 것이 아닌,
		//	  첫 번째로 제출된 Value값이 저장된다.
	 ...
	 * 
	 * @ModelAttribute를 이용해 값이 필드에 저장된 객체를
	 * "커맨드 객체"라고 부른다
	 * 
